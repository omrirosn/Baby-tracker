...
On top of those, `OracleConformanceTests` checks all twelve calculators against
**313 vectors generated by an independent Python implementation** that
formulates every relationship differently from the Swift. Two independent
formulations agreeing to nine significant digits is a much stronger claim than
a test written by the same hand as the code.

## QA

```sh
./Scripts/qa/run-qa.sh              # build, test, UI sweep, accessibility audit
```

Artefacts land in `.qa/`. There is also an agent — `.claude/agents/rf-qa.md` —
that reads them, looks at the screenshots, probes boundaries and writes a
ranked report. See [`docs/QA-AGENT.md`](docs/QA-AGENT.md) for what each layer
catches and, just as importantly, what it does not.
